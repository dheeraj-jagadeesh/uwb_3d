...
def calculate_3d_position(distances, last_known_pos):
    ...
    try:
        ...
        result = least_squares(
            ...
            loss='soft_l1',
            f_scale=5.0  # Softens the penalty for errors beyond 5cm (outliers)
        )
        return result.x
    except Exception:
        return last_known_pos
...
try:
    while plt.fignum_exists(fig.number):
        ...
        # This keeps the data processing real-time and prevents Matplotlib lag from backing up the port.
            ...
            try:
                ...
                if line.startswith("{"):
                    ...
                    if valid_ranges >= 3 and 0 <= t_id <= 5:
                        # Compute coordinates using the faster, robust least_squares method
                        ...
                        # Fetch current local time in HR-MIN-SEC format
        ...
        # Only redraw the graph if we actually processed new coordinate entries
        if data_updated:
            csv_file.flush()  # Push data to disk
            ...
            # Safety check against zero-division errors if tags overlap
            if norm_spine < 1e-5:
        ...
        # Small delay to keep the UI interactive without hanging
        plt.pause(0.001)
