...
# Mapping Tag IDs to friendly names for better data logging readability
TAG_NAMES = {
    0: "Left Wrist",
    1: "Right Wrist",
    2: "Left Knee",
...
# Write comprehensive dataset headers
csv_writer.writerow([
    "Timestamp (s)", "Tag ID", "Tag Name", 
...
try:
            ...
            try:
                ...
                if line.startswith("{"):
                    ...
                    if valid_ranges >= 3 and 0 <= t_id <= 5:
                        ...
                        # Save current telemetry frame to CSV file immediately
                        elapsed_time = time.time() - start_time
                        csv_writer.writerow([
                            f"{elapsed_time:.3f}", t_id, TAG_NAMES.get(t_id, "Unknown"),
                            raw_ranges[0], raw_ranges[1], raw_ranges[2], raw_ranges[3],
                            f"{computed_xyz[0]:.2f}", f"{computed_xyz[1]:.2f}", f"{computed_xyz[2]:.2f}"
                        ])
                        csv_file.flush() # Force write to disk memory buffer
    ...
    # Graceful cleanup to prevent data corruption
    if ser:
        ser.close()
    csv_file.close()
